...
Edited code Author: Kevin Jesubalan (kjesubal@ucsc.edu)
"""
...
class VehicleGeometry():
    ...
    def getNewPoints(self, x, y, z, yaw, pitch, roll):
        ...
        rotation_matrix = Rotations.euler2DCM(yaw, pitch, roll)  # getting the rotation matrix from euler angles

        # Can a TA explain why this code isn't necessary? I fail the test harnesses with it included
        # rotation_matrix = mm.transpose(rotation_matrix)  # changing rotation matrix to be body to vehicle

        newPoints = mm.multiply(newPoints, rotation_matrix)  # applying the rotation matrix to the vertices given
        newPoints = mm.offset(newPoints, x, y, z)
        newPoints = Rotations.ned2enu(newPoints)  # finally we then convert from NED to ENU for the graphic interface
        return newPoints
