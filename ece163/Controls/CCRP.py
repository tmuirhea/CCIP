        ...
        if self.released == 1:
            ...
            if (self.state.pd < 0.0):
                ...
                self.state.u = self.state.u - self.dT * (
                        VPC.rho * self.planArea * self.cofDrag * magnitude * self.state.u) / (2 * self.mass)
                self.state.v = self.state.v - self.dT * (
                        VPC.rho * self.planArea * self.cofDrag * magnitude * self.state.v) / (2 * self.mass)
                self.state.w = self.state.w + self.dT * (
                        VPC.g0 - (VPC.rho * self.planArea * self.cofDrag * magnitude * self.state.w) / (2 * self.mass))
            else:
                self.state.u = 0.0
                self.state.v = 0.0
                self.state.w = 0.0
...
class CCIP:
    ...
    def calculateTOF(self, state, planArea, cofDrag, mass):
        ...
        while z[i] < 0:
            ...
            v.append(v[i] + self.dT * (
                    VPC.g0 - (VPC.rho * planArea * cofDrag * magnitude * v[i]) / (2 * mass)))
    ...
    def Update(self, RefCommand):
        ...
        self.x, self.y = self.payload.calculateFuturePos(self.TOF, self.closed.VAM.vehicle.state.u,
                                                         self.closed.VAM.vehicle.state.v)
        # if targets line up
        print(self.x - self.targetx)
        print(self.y - self.targety)
